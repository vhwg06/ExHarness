...
function anchoredResearchEvidence(ref, body, anchors) {
  ...
  const excerpts=anchors.map((raw,index)=>{
    ...
    const start=Math.max(0,at-220),end=Math.min(body.length,at+match.length+220);
...
export function materialize(root, id, { readiness = false } = {}) {
  ...
  if (lane === 'RESEARCH_SA') {
    ...
    for (let i = 0; i < objective.successCriteria.length; i++) question(`objective-${i}`, `The plan fully covers objective success criterion: ${objective.successCriteria[i]}`, objectiveScopedResearch ? `\`state.objectiveEvidence[${i}]\` including harness-extracted \`groundedSources\`, plus the matching criterion in \`state.objective.successCriteria\`` : '`state.objective` and `state.plan`');
    ...
    const planEvidence = [
      ['blackboard://plan/lane-contract', {
        ...
        objectiveCoverage: objectiveScopedResearch ? plan.objectiveCoverage?.map(({ objectiveCriterion, criterionIds, planElements, verificationIds, decisionIndexes, sourceRefs, sourceAnchors }) => ({ objectiveCriterion, criterionIds, planElements, verificationIds, decisionIndexes, sourceRefs, sourceAnchors })) : plan.objectiveCoverage?.map(({ objectiveCriterion, verificationIds }) => ({ objectiveCriterion, verificationIds }))
      }],
