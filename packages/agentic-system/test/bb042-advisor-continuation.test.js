...
async function withWorkflow({ worker, advisor = null, seen = [] }, run) {
  ...
  try {
    ...
    await run({ orchestrator, makeOrchestrator, makeWorkflow, seen });
  } finally {
...
test("BB-042 terminal cancellation still fences blocked coordination resolution", async () => {
  const worker = backendWorker(async () => result({
    gaps: [{ id: "gap-contract", summary: "Client compatibility contract is missing." }]
  }));
  const advisor = createBackendAdvisor({
  ...
  await withWorkflow({ worker, advisor }, async ({ makeWorkflow }) => {
    ...
    await workflow.advance({ itemId: "BB-042-TASK", owner: "session-a" });
    const canceled = await workflow.cancel({ itemId: "BB-042-TASK", reason: "User canceled." });
    assert.equal(canceled.item.status, BlackboardStatus.SUPERSEDED);

    await assert.rejects(
      () => workflow.resolveBackendCoordination({
        itemId: "BB-042-TASK",
        owner: "coordinator",
        rationale: "Too late.",
        additionalRequiredFiles: ["src/client-contract.md"]
      }),
      /requires BLOCKED item; found SUPERSEDED/
    );
  });
});
