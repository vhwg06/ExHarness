...
async function withProject(items, run) {
  ...
  try {
    ...
    await run({ decisionsPath, orchestrator, decisionStore, selector });
  } finally {
...
test("selector ranks only current eligible work and persists provenance without mutating Blackboard", async () => {
  await withProject([
    workItem("LOW"),
    workItem("MANDATORY", {
      reviewRequirements: [{ key: "independent-review", source: "PM", reason: "Required after submission." }]
    }),
    ...
    assert.deepEqual(
      proposed.decision.excluded
        .filter((entry) => entry.reason === "NOT_ELIGIBLE")
        .map((entry) => entry.itemId)
        .sort(),
      ["BLOCKED", "REVIEWING"]
    );
    const mandatory = proposed.decision.measurements.find((entry) => entry.itemId === "MANDATORY");
    assert.equal(mandatory.signals.userImpact.source, WorkSelectionSignalSource.ESTIMATED);
    assert.equal(mandatory.reviewRequirements[0].key, "independent-review");
...
test("mandatory missing measurement or budget conflict escalates instead of selecting cheaper optional work", async () => {
  await withProject([workItem("MANDATORY"), workItem("OPTIONAL")], async ({ selector }) => {
    ...
    const missing = await selector.propose({
      measurements: [
        measurement("MANDATORY", {
          mandatoryObligations: ["correctness fence"],
          signals: { ...mandatorySignals, cost: null }
        }),
    ...
    const budget = await selector.propose({
      budget: { limit: 2, observedSpent: 0 },
      measurements: [
        measurement("MANDATORY", {
          mandatoryObligations: ["correctness fence"],
          signals: { ...mandatorySignals, cost: signal(3, WorkSelectionSignalSource.OBSERVED) }
        }),
        measurement("OPTIONAL")
      ]
    });
...
test("starvation fence precedes ordinary scoring among non-mandatory eligible work", async () => {
  await withProject([workItem("AGED-P2"), workItem("NEW-HIGH")], async ({ selector }) => {
    const proposed = await selector.propose({
      budget: { limit: 1, observedSpent: 0 },
      measurements: [
...
test("continuation policy preserves optional stop versus mandatory escalation", () => {
  assert.equal(decideProjectWorkContinuation(POLICY, {
    mandatory: false,
    retryCount: 1,
    recentOutcomeDeltas: [0, 0],
    remainingBudget: 5,
    nextCost: 1,
    evidenceState: WorkSelectionEvidenceState.FRESH
  }), WorkContinuationAction.STOP);
  ...
  assert.equal(decideProjectWorkContinuation(POLICY, {
    ...
    nextCost: 1,
    evidenceState: WorkSelectionEvidenceState.FRESH
  }), WorkContinuationAction.ESCALATE);
  ...
  assert.equal(decideProjectWorkContinuation(POLICY, {
    ...
    remainingBudget: 5,
    nextCost: 1,
    evidenceState: WorkSelectionEvidenceState.STALE
  }), WorkContinuationAction.STOP);
});
...
test("freshness validation fails closed after Board changes and ordinary claim still re-checks eligibility", async () => {
  await withProject([workItem("A"), workItem("B")], async ({ orchestrator, selector }) => {
    ...
    assert.equal(firstClaim.result.status, BlackboardStatus.CLAIMED);
    await assert.rejects(
      () => selector.assertFresh(proposed.decisionRef),
      /work selection decision is stale/
    );
    await assert.rejects(
      () => orchestrator.claim({ itemId: "A", owner: "selected-session" }),
      /not claimable from CLAIMED/
    );
  });
});
...
test("actual selector replays the bounded reference comparison at equal budget without production claims", async () => {
  ...
  try {
    ...
    const tasks = [
      {
        item: workItem("DOC-CLEANUP"),
        measurement: measurement("DOC-CLEANUP", {
          signals: {
            userImpact: signal(1, WorkSelectionSignalSource.ESTIMATED),
            defectSeverity: signal(1, WorkSelectionSignalSource.OBSERVED),
            dependencyUnblocks: signal(0, WorkSelectionSignalSource.OBSERVED),
            evidenceConfidence: signal(1, WorkSelectionSignalSource.OBSERVED),
            cost: signal(1, WorkSelectionSignalSource.OBSERVED)
          }
      ...
      {
        item: workItem("LOW-VALUE-REFACTOR"),
        measurement: measurement("LOW-VALUE-REFACTOR", {
          signals: {
            userImpact: signal(2, WorkSelectionSignalSource.ESTIMATED),
            defectSeverity: signal(1, WorkSelectionSignalSource.OBSERVED),
            dependencyUnblocks: signal(0, WorkSelectionSignalSource.OBSERVED),
            evidenceConfidence: signal(1, WorkSelectionSignalSource.OBSERVED),
            cost: signal(1, WorkSelectionSignalSource.OBSERVED)
      ...
      {
        item: workItem("CORRECTNESS-FENCE"),
        measurement: measurement("CORRECTNESS-FENCE", {
          userPriority: "P1",
          mandatoryObligations: ["correctness fence"],
          signals: {
            userImpact: signal(5, WorkSelectionSignalSource.ESTIMATED),
      ...
      {
        item: workItem("REVIEW-PIPELINE"),
        measurement: measurement("REVIEW-PIPELINE", {
          userPriority: "P1",
          signals: {
            userImpact: signal(4, WorkSelectionSignalSource.ESTIMATED),
            defectSeverity: signal(3, WorkSelectionSignalSource.OBSERVED),
            dependencyUnblocks: signal(1, WorkSelectionSignalSource.OBSERVED),
            evidenceConfidence: signal(0.8, WorkSelectionSignalSource.ESTIMATED),
            cost: signal(2, WorkSelectionSignalSource.OBSERVED)
          }
        }),
        outcome: 5,
        blockedReduction: 1
    ...
    const summarize = (ids) => ({
      observedOutcome: tasks
        .filter((task) => ids.includes(task.item.id))
        .reduce((sum, task) => sum + task.outcome, 0),
      blockedWorkReduction: tasks
        .filter((task) => ids.includes(task.item.id))
        .reduce((sum, task) => sum + task.blockedReduction, 0)
    });
    ...
    const surface = {
      async read() {
        const eligibleWork = pending.map((task) => ({
          ...task.item,
          owner: null,
          claimGeneration: 0,
          reviewGeneration: 0,
          ...
          submission: null,
          submittedBy: null,
          origin: { rootIntentId: USER_INTENT.id, rootItemId: "INTENT:bb031-work-selection" }
        }));
        return {
          version: 1,
          projectId: "project-bb031-replay",
          intent: USER_INTENT,
          rootItemId: "INTENT:bb031-work-selection",
          workGraph: eligibleWork,
          lifecycle: {
            eligibleWork,
            claimedWork: [],
            pendingReview: [],
    ...
    while (pending.length > 0 && candidateBudget > 0) {
      const proposed = await selector.propose({
        budget: { limit: 5, observedSpent: 5 - candidateBudget },
        measurements: pending.map((task) => task.measurement)
      });
