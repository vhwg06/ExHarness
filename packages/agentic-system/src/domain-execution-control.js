...
const headRevision=value=>hash(value);
...
export function createDomainExecutionController({claimController,claimReleaseStore,organizationArtifactRegistry,artifactRegistry,executionPolicyStore,executionAttemptStore,runtimeAdapter,completionEvaluator,publicationGate}){
  ...
  inv(typeof publicationGate?.withCurrentWriteAuthority==="function","publication write-authority guard required");
  inv(typeof publicationGate?.publishIdempotent==="function","idempotent publication gate required");
  ...
  async function recoverHead(x){
    if(x.head.value.status!==ExecutionAttemptStatus.ACTIVE)return x;
    const observed=x.head.revision;
    const tr=await transition(artifactRegistry,{workId:x.binding.workId,id:x.binding.executionAttemptId,from:ExecutionAttemptStatus.ACTIVE,to:ExecutionAttemptStatus.RECOVERY_REQUIRED,reason:"RECOVERY_REQUIRED",decisionRef:x.bindingRef,observedHeadRevision:observed});
    const next={...x.head.value,status:ExecutionAttemptStatus.RECOVERY_REQUIRED,transitionRefs:[...x.head.value.transitionRefs,tr]};
    if(await executionAttemptStore.compareAndSwap(x.key,observed,next))x.head=await executionAttemptStore.current(x.key);
    else{
      const current=await executionAttemptStore.current(x.key);
      inv(current?.value?.executionAttemptId===x.binding.executionAttemptId,"attempt changed during recovery");
      inv([ExecutionAttemptStatus.RECOVERY_REQUIRED,ExecutionAttemptStatus.TERMINAL].includes(current.value.status),"recovery CAS lost to incompatible attempt state");
      x.head=current;
    }
    return x;
  }
  ...
  async function execute({itemId,claimGeneration,receiptRef,input={}}){const s=await subject({claimController,claimReleaseStore,organizationArtifactRegistry,itemId,claimGeneration,receiptRef});let x=await establish(s,{itemId,claimGeneration,receiptRef});if(x.head.value.status===ExecutionAttemptStatus.TERMINAL)return terminal(x.head);const recovery=!x.created;if(recovery){x=await recoverHead(x);if(x.head.value.status===ExecutionAttemptStatus.TERMINAL)return terminal(x.head);}await claimController.assertExecutable({itemId,claimGeneration,receiptRef});const bound=await artifactRegistry.resolveExecutionStrategyDescriptor(x.binding.executionStrategyRef);await strategy({artifactRegistry,p:{strategyRef:x.binding.executionStrategyRef},contract:s.contract,runtime});let result;try{result=recovery?await runtimeAdapter.recover({binding:x.binding,input:fr(input),runtimeInvocationKey:x.binding.runtimeBinding.runtimeInvocationKey}):await runtimeAdapter.dispatch({binding:x.binding,input:fr(input),runtimeInvocationKey:x.binding.runtimeBinding.runtimeInvocationKey});}catch(e){if(x.head.value.status===ExecutionAttemptStatus.ACTIVE)x=await recoverHead(x);throw e;}inv(bound&&result&&["SUCCEEDED","FAILED","BLOCKED","UNKNOWN"].includes(result.status),"runtime result invalid");
    ...
    if(a.verdict==="ACCEPT"){
      const publicationKey="domain-publication:"+hash({executionAttemptId:x.binding.executionAttemptId,bindingRef:x.bindingRef});
      const guarded=await claimController.withExecutablePublicationGuard({itemId,claimGeneration,receiptRef},async(lifecycleObservation)=>{
        return publicationGate.withCurrentWriteAuthority({
          domain:s.contract.owningDomain,
          producerPrincipalRef:publicationPrincipal
        },async(writeAuthorityObservation)=>{
          inv(writeAuthorityObservation&&typeof writeAuthorityObservation==="object","write authority observation required");
          const writeAuthorityRef=txt(writeAuthorityObservation.authorityRef??publicationAuthority,"write authority ref");
          const writeAuthorityRevision=txt(writeAuthorityObservation.revision,"write authority revision");
          const p=await publicationGate.publishIdempotent({
            publicationKey,
            contract:s.contract,
            binding:x.binding,
            bindingRef:x.bindingRef,
            outcome,
            outcomeRef,
      ...
      const p=guarded.result.p;
      const published=arr(p.publishedArtifactRefs??[],"published artifacts").map(o=>fr({ref:txt(o.ref,"published ref"),digest:txt(o.digest,"published digest")}));
      inv(published.every(y=>outputs.some(o=>o.ref===y.ref&&o.digest===y.digest)),"publication invented output");
      const accepted=arr(p.acceptedDerivationEdges??[],"accepted edges").map(e=>fr({outputRef:txt(e.outputRef,"accepted output"),derivedFrom:strs(e.derivedFrom,"accepted inputs")}));
      inv(accepted.every(e=>edges.some(c=>c.outputRef===e.outputRef&&e.derivedFrom.every(r=>c.derivedFrom.includes(r)))),"publication invented lineage");
      inv((p.publicationKey??publicationKey)===publicationKey,"publication idempotency key mismatch");
      publicationRef=await artifactRegistry.putDomainPublicationReceipt(fr({
        kind:"DOMAIN_PUBLICATION_RECEIPT",version:1,
        domain:s.contract.owningDomain,
        producerPrincipalRef:publicationPrincipal,
        writeAuthorityRef:guarded.result.writeAuthorityRef,
        writeAuthorityRevision:guarded.result.writeAuthorityRevision,
        lifecycleObservation:guarded.result.lifecycleObservation,
        publicationKey,
        completionDecisionRef:decisionRef,
        publishedArtifactRefs:published,
        publishedClaimRefs:arr(p.publishedClaimRefs??[],"published claims"),
        acceptedDerivationEdges:accepted,
...
export async function resolveExecutionJudgmentBundle({artifactRegistry,organizationArtifactRegistry,bundleRef}){
  ...
  let expectedHeadRevision=null;
  const observedTransitionRefs=[];
  for(let index=0;index<transitions.length;index+=1){
    ...
    inv((t.observedHeadRevision??null)===expectedHeadRevision,"attempt transition observed head revision mismatch");
    observedTransitionRefs.push(p.transitionHistory[index].ref);
    transitionStatus=t.toStatus;
    if(transitionStatus!==ExecutionAttemptStatus.TERMINAL){
      expectedHeadRevision=headRevision({
        status:transitionStatus,
        executionAttemptId:binding.executionAttemptId,
        bindingRef:p.binding.ref,
        transitionRefs:[...observedTransitionRefs],
        outcomeRef:null,
  ...
  if(publication){
    ...
    inv(publication.publicationKey==="domain-publication:"+hash({executionAttemptId:binding.executionAttemptId,bindingRef:p.binding.ref}),"publication key relation mismatch");
    inv(typeof publication.writeAuthorityRevision==="string"&&publication.writeAuthorityRevision.length>0,"publication write-authority revision missing");
    inv(publication.lifecycleObservation?.workContractRef===contract.contractRef&&publication.lifecycleObservation?.claimRelease?.receiptRef!=null,"publication lifecycle observation mismatch");
