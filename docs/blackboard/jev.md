...
`blackboard-jev.yml` is separate from the Node test matrix. Manual dispatch accepts a current work id. Automatic Jev routing happens only after the successful same-repository **pull-request** `test` workflow, which contains both the kernel verification and Living-doc impact gate. Pushes run the separate `push-test` kernel workflow and do not create automatic Jev evaluations. WORKER is selected only when its bound candidate SHA equals the tested PR head. RESEARCH_SA is selected only when the tested PR head changes the current schedulable plan draft or its exact `researchBaselineSha`. Unrelated PRs produce an explicit `no Jev evaluation required` job rather than being interpreted as a Jev pass.

For automatic PR evaluation, the workflow pins the trusted checkout to the PR base SHA and the subject checkout to the exact tested PR head; downstream jobs reuse that exact trusted SHA rather than floating to a newer `main`. RESEARCH_SA may contribute only its canonical plan draft and exact research baseline; objective, routing, evaluator policy and evaluator code remain trusted-base data. One job runs verification without TypeSafe credentials. A separate job reads the candidate data, calls Jev, validates the typed result, and uploads `jev-evaluation-<WORK_ID>`. A canonical readiness-publication PR does not call the provider again: it runs `verify published Jev readiness`, which validates the committed SATISFIED evaluation against the exact trusted readiness input. It never executes candidate code with the provider key.
