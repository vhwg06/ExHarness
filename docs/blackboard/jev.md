...
`blackboard-jev.yml` is separate from the Node test matrix. Manual dispatch accepts a current work id. A successful same-repository verification workflow automatically selects current WORKER tasks only when their bound candidate SHA equals that workflow head. For RESEARCH_SA, it selects only a schedulable current task whose exact plan draft or `researchBaselineSha` changed on that tested head (including the merge commit that introduces the research change). Unrelated green test runs do not claim an unbound research task.

The workflow checks out routing/evaluator code and current task identity from trusted `main`, plus the exact tested head as a read-only subject. RESEARCH_SA may contribute only its canonical plan draft and exact research baseline; objective, routing, evaluator policy and evaluator code remain trusted-main data. One job runs verification without TypeSafe credentials. A separate job reads the candidate data, calls Jev, validates the typed result, and uploads `jev-evaluation-<WORK_ID>`. It never executes candidate code with the provider key. Only same-repository successful workflow runs are automatically eligible.
