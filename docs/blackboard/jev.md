...
The router pins three independent identities: the current audited controller/evaluator revision, the exact PR-base control-plane snapshot resolved from GitHub by tested head SHA, and the exact tested subject head. Its dispatch carries all three immutable SHAs into `blackboard-jev`. `collect Jev evidence` and `Jev evaluate` execute code from the pinned controller revision while reading routing/objective authority from the pinned PR-base snapshot and candidate data from the subject. RESEARCH_SA may contribute only its canonical plan draft and exact research baseline; objective, routing, evaluator policy and evaluator code remain trusted-PR-base data. Evidence collection runs without TypeSafe credentials; `Jev evaluate` alone receives `TYPESAFE_API_KEY`, calls the provider, validates the typed result and uploads `jev-evaluation-<WORK_ID>`. A canonical readiness-publication PR is handled by the router's `verify published Jev readiness` job instead of dispatching a second provider call.
