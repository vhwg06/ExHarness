...
The pull-request `test` workflow is the only automatic semantic-gate surface. After kernel verification and the Living-doc impact gate pass, its internal `blackboard-gate` job selects exact WORKER/RESEARCH_SA judgment subjects or readiness publications. If semantic judgment is required, the same PR run invokes the trusted reusable `blackboard-jev.yml@main`; there is no standalone router workflow or dispatch chain. Pushes run the separate `push-test` kernel workflow and do not create automatic Jev evaluations.

`blackboard-gate` reads routing/objective authority from the exact PR-base checkout and candidate data from the exact PR head. The reusable Jev workflow receives exact work id, candidate SHA, trusted PR-base SHA and controller SHA; `collect Jev evidence` runs without TypeSafe credentials, while `Jev evaluate` alone receives `TYPESAFE_API_KEY`, calls the provider, validates the typed result and uploads `jev-evaluation-<WORK_ID>`. RESEARCH_SA may contribute only its canonical plan draft and exact research baseline. A canonical readiness-publication PR runs the sibling `readiness-verify` job in the same PR workflow instead of rerolling the provider.
