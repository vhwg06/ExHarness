...
Acceptance boundary: repository owner explicitly required living docs to be updated continuously so they describe the system as closely as possible at every completed implementation checkpoint.
...
The projection does not need to mirror source text mechanically, but it must remain semantically aligned with the behavior, architecture, contracts, boundaries and workflow that actually exist now.

## Same-change rule

Any change that alters current behavior, architecture, contracts, authority boundaries, public surface or executable workflow must reconcile the affected `docs/worktree/*` documents in the same change before that change is eligible for acceptance/merge.
...
Do not defer this reconciliation until:
...
A work item may remain unresolved while its current implemented partial state is already reflected in living docs.
...
The Blackboard must not be used to justify stale living docs. Blackboard stores unresolved work; worktree docs describe what exists now, including partial current implementation when material.
...
Review must explicitly assess living-doc impact:
...
CURRENT_SYSTEM_NOT_CHANGED
  -> no worktree rewrite required; reviewer can verify the change is documentation-neutral
```
