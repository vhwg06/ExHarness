...
Outer repository-development tooling is separate from the Agentic Application runtime. Its current contract routes OBJECTIVE through RESEARCH_SA to READY_IMPLEMENT_PLAN, then through WORKER to DELIVERED_FEATURE. The scripts validate exact semantic bindings, collect candidate evidence, integrate typed Jev evaluation and verify candidate presence/tree identity on main. Automatic Jev routing occurs only after the successful same-repository pull-request `test` workflow, where kernel verification and the Living-doc impact gate both run; push commits use a separate `push-test` kernel workflow and do not imply a Jev or Living-doc gate. WORKER is selected only by exact bound candidate SHA, while RESEARCH_SA is selected only when the tested PR head changes the current schedulable plan draft or its exact research baseline. Trusted evaluator/routing data is pinned to the PR base SHA and the subject to the exact PR head, so reruns do not float onto newer main state. A canonical `RESEARCH_SA/SATISFIED` readiness publication advances that same semantic plan to `WORKER/EXECUTION` through an explicit readiness-publication verification job rather than another provider call; unrelated PRs report `no Jev evaluation required` instead of being treated as Jev success. Other routing mutations remain rejected. Large baseline files are sent as deterministic excerpt bodies bounded below 2 Ki characters per file while full-file hashes remain in the semantic state, preventing omitted source changes from reusing a stale Jev result. Live provider judgment and actual delivery require their own evidence; offline tooling tests do not establish either. Operational details are in `docs/blackboard/jev.md`.
