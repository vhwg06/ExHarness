...
BB-065 now contains a separate `FIXTURE_VALUE_V1` study path for the six-pair, twelve-execution Core comparison. Its registration is distinct from the old three-task calibration and future twenty-pair pilot. `resource-state.mjs` persists hash-chained request/attempt/recovery events, bounded waits, non-admission proofs, UNKNOWN reconciliation and portable current handoff; the provider ledger reserves each wire attempt and will not clear uncertain usage. `study.mjs` reconstructs registered executions before dispatch, runs two bounded live tool-call probes first, uses one common mini-SWE-agent executor for DIRECT and EXHARNESS, and records factual paired metrics only. `core-arm.mjs` uses the current Core `observe`, `act`, `recordVerification`, `evaluate` and `promote` APIs with file-backed session state; a no-op cannot promote the baseline. `jev-value.mjs` materializes independent stage-one dimensions and a dependent final Choice. Its append-only journal reserves up to 64,000 input tokens at the registered price per wire attempt, enforces two stages, two transport attempts per stage, the 128,000-token/$0.02 totals, and retains worst-case usage for ambiguous attempts. Only the real bounded Jev transport can produce a LIVE receipt; injected test doubles are explicitly deterministic. Judgment spend is reported separately and included in known whole-study API cost. The study report contains no code-authored value verdict; it exposes `valueEvaluationRef` for the trusted Jev receipt. Test output and earlier diagnostic cohorts do not establish a value conclusion; inspect the exact registered cohort and audited Jev receipt for any benchmark claim. The fixture result is scoped to these tasks and does not establish a product-wide claim.
