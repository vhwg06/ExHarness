...
Outer repository-development tooling is separate from the Agentic Application runtime. Its current contract routes OBJECTIVE through RESEARCH_SA to READY_IMPLEMENT_PLAN, then through WORKER to DELIVERED_FEATURE. The scripts validate exact semantic bindings, collect candidate evidence, integrate typed Jev evaluation and verify candidate presence/tree identity on main. Successful same-repository verification may automatically route Jev: WORKER is selected only by exact bound candidate SHA, while RESEARCH_SA is selected only when the tested head changes the current schedulable plan draft or its exact research baseline. Research evaluation keeps objective/routing/evaluator policy on trusted main and imports only the exact plan draft plus research baseline from the tested head. A canonical `RESEARCH_SA/SATISFIED` readiness publication that advances that same semantic plan to `WORKER/EXECUTION` is treated as control-plane publication and is not re-evaluated as a fresh research candidate; other routing mutations remain rejected. Large baseline files are sent as deterministic excerpt bodies bounded below 2 Ki characters per file while full-file hashes remain in the semantic state, preventing omitted source changes from reusing a stale Jev result. Live provider judgment and actual delivery require their own evidence; offline tooling tests do not establish either. Operational details are in `docs/blackboard/jev.md`.
