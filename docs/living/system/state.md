...
The development-only delivery baseline under `scripts/delivery/baseline/` provides a pinned Node/SQLite/vanilla-browser request-tracker fixture, three seeded calibration changes, a Docker-isolated direct mini-SWE-agent adapter, a separate API/browser/persistence verifier, provider-budget accounting, and a frozen 20-pair pilot registration/report contract. Deterministic tests exercise those boundaries. A LIVE calibration requires an externally supplied OpenAI API key, three real `gpt-6-luna` agent runs and independently retrieved provider usage; deterministic results are never presented as provider evidence. The baseline has no product runtime, acceptance-publication authority, or completed comparative value claim. The later pilot context and customer task choices remain product-owner decisions.
