...
The development-only delivery baseline under `scripts/delivery/baseline/` provides a pinned Node/SQLite/vanilla-browser request-tracker fixture, three seeded calibration changes, a Docker-isolated direct mini-SWE-agent adapter, a separate API/browser/persistence verifier, provider-budget accounting, and a frozen 20-pair pilot registration/report contract. Deterministic tests exercise those boundaries. LIVE calibration targets NVIDIA NIM's Nemotron 3.5 Lightning endpoint; the committed profile identifies the catalog release without claiming a hosted backend revision. The driver maps a local `NVIDIA_API_KEY` to `NVIDIA_NIM_API_KEY` in memory. Original Chat Completions responses and usage are captured and hashed at response time; only HTTP 202 responses use status polling. Missing cache detail stays unreported, while missing token usage remains unknown and fences later calls for that task. This is source behavior, not evidence that a LIVE calibration completed. The synthetic fixture may be sent to the provider, and agent exit status remains separate from verifier acceptance. The baseline has no product runtime, acceptance-publication authority, or completed comparative value claim. The later pilot context and customer task choices remain product-owner decisions.

Outer repository-development tooling is separate from the Agentic Application runtime. Its current contract routes OBJECTIVE through RESEARCH_SA to READY_IMPLEMENT_PLAN, then through WORKER to DELIVERED_FEATURE. The scripts validate exact semantic bindings, collect candidate evidence, integrate typed Jev evaluation and verify candidate presence/tree identity on main. The pull-request `test` workflow is the single automatic semantic-gate surface: kernel verification and Living-doc impact run first, then an internal `blackboard-gate` selects exact judgment work or readiness publication. Exact semantic judgment invokes the trusted reusable `blackboard-jev.yml@main` inside the same PR run with pinned work id, PR-base authority SHA and subject SHA; there is no standalone Blackboard router or workflow-dispatch chain. Canonical `RESEARCH_SA/SATISFIED` readiness publication is checked by a sibling `readiness-verify` job rather than rerolling the provider. Push commits use only the separate `push-test` kernel workflow. WORKER is selected only by exact bound candidate SHA. RESEARCH_SA is selected when the tested PR head changes the current plan draft or its exact research baseline even if execution dependencies are still non-DONE; dependency readiness gates WORKER execution, not research planning or Jev readiness. Research objective judgment supports an opt-in `OBJECTIVE_SCOPED_V1` evidence route: opted-in plans give each objective question its exact objectiveCoverage binding (criterionIds, planElements, verificationIds), selected decision/source identifiers and an allowlisted set of objective-specific supporting plan fields. Criterion/slice/decision bodies remain canonical once in `state.plan`; objective evidence references them by ID instead of duplicating their full text. Supporting fields are copied only when named by that objective's coverage entry; arbitrary plan extensions remain excluded. Objective-scoped research may additionally bind bounded exact `sourceAnchors`; the trusted harness resolves each anchor against the pinned research baseline and supplies only the surrounding source excerpt plus full-file hash/byte identity. Generic objective-scoped source evidence also enforces its declared compact character budget rather than merely switching to a fixed compact mode, so grounding does not silently overflow the provider payload. Plans without that routing contract retain the legacy materialization byte-for-byte so already accepted readiness evidence does not become stale when the evaluator evolves. Other routing mutations remain rejected. Large baseline files are sent as deterministic excerpt bodies bounded below 2 Ki characters per file while full-file hashes remain in the semantic state, preventing omitted source changes from reusing a stale Jev result. Live provider judgment and actual delivery require their own evidence; offline tooling tests do not establish either. Operational details are in `docs/blackboard/jev.md`.
...
The calibration runner is not a supported product deployment and does not establish measured ExHarness-versus-direct value uplift. The separate Core study below supplies the development-only treatment adapter. LIVE calibration requires fresh provider, agent, verifier and accounting evidence followed by independent judgment; source code and earlier diagnostic runs alone do not establish delivery.

The development delivery baseline now has a separate `CORE_VALUE_V2` study path for the six-pair, twelve-execution Core comparison; the old three-task calibration and future twenty-pair pilot remain distinct registrations. `qualification.mjs` qualifies the existing NVIDIA route before registration with two settled bash probes, retains all failed attempts, and binds the selected thinking-disabled/tool-choice profile to the cohort. `resource-state.mjs` persists hash-chained request/attempt/recovery events, bounded waits, non-admission proofs, UNKNOWN reconciliation and portable current handoff; the provider ledger reserves each wire attempt and will not clear uncertain usage. `study.mjs` reconstructs registered executions before dispatch, runs bounded live tool-call probes first, uses one common mini-SWE-agent executor for DIRECT and EXHARNESS, and records factual paired metrics only. `core-arm.mjs` uses the current Core `observe`, `act`, `recordVerification`, `evaluate` and `promote` APIs with cohort-scoped file-backed session state and a verified durable state hash; a no-op cannot promote the baseline. `controlled-trials.mjs` adds 24 deterministic production-seam replays across four frozen fault scenarios, three task families and both arms; these never call a model and cannot replace the twelve live executions. `jev-value.mjs` materializes compact evidence within the registered bound, asks Jev-only evidence/comparison/quality/efficiency/candidate-control/recovery dimensions, then asks a dependent net-value Choice. Only the real bounded Jev transport can produce a LIVE receipt; injected test doubles are explicitly deterministic. Receipt audit is separate from benchmark completion: `complete` requires all twelve settled/verified live rows, all 24 controlled rows and a definitive `VALUE_DEMONSTRATED` or `NO_VALUE_DEMONSTRATED` answer. No live value conclusion has been established by source or deterministic tests; the benchmark is scoped to these fixture tasks and does not establish a product-wide claim.

Current delivery-baseline repair keeps route-pacing waits inside the same mini process and attempt, records their elapsed time separately, and passes positive integer mini deadlines on retries. Jev receives the actual live/controlled observations in `state.study`, including explicit column definitions and hash-bound request summaries with failures; transport tests check both stages, not just an unsent local evidence envelope. Qualification validation no longer creates a CLI module-evaluation cycle, and completion loads the same probe evidence used by evaluation. These repairs do not reconcile prior UNKNOWN provider calls or validate earlier sparse-input Jev receipts as value evidence. A newly bound complete live study and real Jev conclusion are still required.

The delivery-baseline implementation S0–S4 (canary gate, cumulative waits with a single POST→poll deadline, cohort-scoped sessions, phase timings, shared DIRECT seam with real fault-injection controlled trials, file-backed audits, Jev neutrality with verbatim stage-two inside the 24 KB bound) is committed at `9b4521f` with 72 deterministic tests passing. Live sequence on the frozen candidate: qualification READY (profile A, two settled bash probes, cohort `BB065-CORE-VALUE-2:98d462d95446`), then the preregistered runtime-readiness canary FAILED and was audit-confirmed: three interleaved tool turns and the exact marker were produced with four settled wire requests and zero UNKNOWN usage, but the run ended `LimitsExceeded` without submitting, so the submit requirement was not met. Registration was refused, zero cohort executions were dispatched, and no manifest exists. There is no live cohort, no controlled-trial run, and no Jev value verdict of any kind; the canary FAIL record and journals are retained under the delivery baseline runs directory. This is an explicit prerequisite failure, not a value finding in either direction.
