...
**Guarantees:** organization-claim lifecycle mutations that can invalidate publication and guarded publication share one non-takeover Blackboard mutation fence, while unrelated Board mutations retain the immutable optimistic single-successor revision chain; publication uses a stable semantic-attempt publication key; concurrent recovery/retry converges on one canonical publication; the publication receipt records the exact lifecycle and writer-authority observations used by the mutation gate; recovery-relevant attempt transitions bind the exact observed durable head revision; fresh judgment re-derives transition/CAS revision continuity.
