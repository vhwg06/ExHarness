...
After `DOMAIN_COMPLETION_DECISION.verdict == ACCEPT`, publication is a separate mutation-current transaction. The controller enters an exact organization-claim lifecycle guard backed by the public Blackboard store's shared mutation fence, then an exact current domain writer-authority guard, and performs the canonical write while both remain held. Every normal Blackboard `transact()` acquires that same fence before loading its committed head and retains it through successor publication; guarded publication acquires it before reading the lifecycle subject and retains it through the external canonical write. The immutable single-successor revision chain remains commit authority; elapsed time never grants a second fence owner, and fence contention waits/fails closed rather than stealing ownership.
