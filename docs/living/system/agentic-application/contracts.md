...
After `DOMAIN_COMPLETION_DECISION.verdict == ACCEPT`, publication is a separate mutation-current transaction. The controller enters an exact organization-claim lifecycle guard, then an exact current domain writer-authority guard, and performs the canonical write while both remain held.
