...
Research revision: **v8 explicit lifecycle + artifact-evolution clarification**.

This revision preserves the v7 trust/authority architecture. It does **not** rebase B→J, introduce a workflow-generation engine, or move organizational authority into Restate. It makes explicit an already-compatible delivery assumption: lifecycle/workload workflows may be predefined and versioned, while the project artifact set may begin incomplete and evolve with the end user across those known phases.
