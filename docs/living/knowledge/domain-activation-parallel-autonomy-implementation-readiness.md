...
This artifact is the canonical Researcher + SA architecture input for the first autonomous multi-domain execution slice after Integration C.
...
Promotion of this research to `main` does not allocate Integration D work and does not authorize source changes.

After Integration C is accepted/current and a grounded Integration D trigger exists:

```text
promoted DOMAIN_ACTIVATION research
  -> allocate Blackboard item
  -> read-only REVIEW generation against exact then-current source baseline
  -> accepted decision bound to review context/candidate
  -> child IMPLEMENT generation with exact write/test scope
  -> READY -> worker
```

No `work-artifacts/*` staging layer is part of the outer Blackboard contract.
