...
test('research objective questions receive bounded objective-to-criterion-to-slice evidence',t=>{
  ...
  assert.deepEqual(parsed.objectiveCoverage,[{objectiveCriterion:'answer is two',criterionIds:['AC1'],planElements:['D1'],verificationIds:['unit'],decisionIndexes:[0],sourceRefs:['source.js'],sourceAnchors:[{ref:'source.js',match:'export const answer = 1;'}]}]);
});

test('objective source anchors fail closed when baseline text is missing',t=>{
  const f=fixture(t);
  const plan=read(f.root,'plan.json');
  plan.jevEvidenceRouting='OBJECTIVE_SCOPED_V1';
  plan.implementationSlices=['D1 — Change constant'];
  plan.objectiveCoverage[0].sourceAnchors=[{ref:'source.js',match:'missing exact baseline text'}];
  write(f.root,'plan.json',plan);
